...
def pendulum_ode(s: Array, a: Array) -> Array:
    ...
    return jnp.stack((dq, ddq))


trans_noise = MultivariateNormalDiag(jnp.zeros(2), scale_diag=jnp.array([0.0, 0.1]))
...
def reward_fn(s: Array, a: Array) -> Array:
    ...
    def wrap_angle(q: Array) -> Array:
        return jnp.array((jnp.mod(q[0], (2 * jnp.pi)), q[1]))
...
prior_dist = MultivariateNormalDiag(
    loc=jnp.zeros((state_dim,)), scale_diag=jnp.ones((state_dim,)) * 1e-8
)
...
key, sub_key = random.split(key)
traced_outer_states, _ = backward_tracing(sub_key, outer_states, inner_states)

key, sub_key = random.split(key)
random_idx = random.choice(sub_key, num_outer_particles)
angles = traced_outer_states.particles.observations[:, :, 0]
angular_velocities = traced_outer_states.particles.observations[:, :, 1]
...
for i in range(1, num_epochs + 1):
    ...
    # run interleaved smc
    key, sub_key = random.split(key)
    outer_states, inner_states, norm_const = smc(
        sub_key,
        num_outer_particles,
        num_inner_particles,
        num_time_steps,
    ...
    for batch_idx in batch_indices:
        outer_states_batch = jax.tree.map(
            lambda x: x[:, batch_idx], traced_outer_states.particles
        )
        train_state, batch_loss = train_step(train_state, policy, outer_states_batch)
        loss += batch_loss

    print(
        f"Iter: {i}, Loss: {loss}, Normalization constant: {norm_const}, Return: {cumulative_return}"
    )
...
state = jnp.array([0.0, 0.0])  # prior_dist.sample(seed=state_key, sample_shape=(1,))
