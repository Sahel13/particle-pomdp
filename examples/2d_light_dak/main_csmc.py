...
jitted_csmc = jax.jit(smc, static_argnums=(1, 2, 3, 4, 5, 6, 7, 9, 10))
...
outer_reference = jax.tree.map(lambda x: x[:, idx], traced_outer)
inner_reference = jax.tree.map(lambda x: x[idx], traced_inner)
reference = Reference(
    observations=outer_reference.particles.observations,
    actions=outer_reference.particles.actions,
    carry=outer_reference.particles.carry,
    log_probs=outer_reference.particles.log_probs,
