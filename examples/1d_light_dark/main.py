...
for n in range(num_outer_particles):
    ...
    axs[1].plot(actions[:, n, :])
    axs[1].set_title('Action over Time')
    axs[1].set_xlabel('Time Step')
    axs[1].set_ylabel('Action')

    axs[2].plot(observations[:, n, :])
...
#     # run nested smc
#     key, sub_key = random.split(key)
#     outer_states, inner_states, inner_info, log_marginal = jitted_smc(
#         sub_key,
#         num_outer_particles,
#         num_inner_particles,
#         num_time_steps,
...
#         f"Entropy: {entropy:8.3f}, "
#         f"Time per epoch: {time_diff:6.3f}s"
#     )
#
#
