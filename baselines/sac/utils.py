...
from baselines.sac.core import PRNGKey, Env, EnvState, JointTrainState
from baselines.sac.arch import CriticNetwork, PolicyNetwork, policy_sample_and_log_prob
...
def step_env(
    ...
    def _false_fn(_outer_state):
        return init_env(rng_key, env_obj, policy_state, random_actions)
...
def create_train_state(
    ...
    policy_std = jnp.ones(env_obj.action_dim)
    policy_network = PolicyNetwork(
        feature_fn=env_obj.feature_fn,
        time_norm=env_obj.num_time_steps,
        layer_sizes=(256, 256, env_obj.action_dim),
        init_log_std=nn.initializers.constant(policy_std),
    )
...
def step_and_train(
    ...
    def body(carry, rng_keys):
        ...
        _buffer_state, _env_state_sample = buffer_obj.sample(_buffer_state)
        _train_state, _, _ = \
            gradient_step(
                rng_key=_train_key,
        ...
        return (_env_state, _buffer_state, _train_state), None

    keys = random.split(rng_key, num_steps * 2).reshape((num_steps, 2))
    (env_state, buffer_state, train_state), _ = \
        jax.lax.scan(
            body, (env_state, buffer_state, train_state), keys
        )
    return env_state, buffer_state, train_state
